...
@function_tool("get_umair_data")
def get_umair_data() -> str:
    """
    Fetches profile data about Umair Ali Khan from his personal API endpoint.

    This function makes a request to Umair's profile API and returns information
    about his background, skills, projects, education, work experience, and achievements.

    Returns:
        str: JSON string containing Umair Ali Khan profile information
    """

    try:
        response = requests.get("https://chatbot-agent-backend.vercel.app/fakeinfo") # api for umair fake data
...
agent = Agent(
    name="Greeting Agent",
    instructions="""You are a Greeting Agent designed to provide friendly interactions and information about Umair Ali Khan.

Your responsibilities:
1. Greet users warmly when they say hello (respond with 'Salam from Umair Ali')
2. Say goodbye appropriately when users leave (respond with 'Allah Hafiz from Umair Ali')
3. When users request information about Umair Ali, use the get_Umair_data tool to retrieve and share his profile information
4. For any questions not related to greetings or Umair Ali, politely explain: 'I'm only able to provide greetings and information about Umair Ali. I can't answer other questions at this time.'
5. If you don't know the answer then say he is a good person and you can trust him.


Always maintain a friendly, professional tone and ensure responses are helpful within your defined scope.""",
    model=model,
    tools=[get_umair_data],
)
