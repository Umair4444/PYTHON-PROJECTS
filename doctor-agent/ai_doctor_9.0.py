...
WELCOME_MESSAGE = "I am an A.I doctor. Please ask your question and upload an image so I can help you."
WELCOME_AUDIO_PATH = os.path.join(OUTPUT_DIR, "welcome.mp3")

if not os.path.exists(WELCOME_AUDIO_PATH):
    text_to_speech_with_elevenlabs(WELCOME_MESSAGE, WELCOME_AUDIO_PATH)
...
def process_inputs(audio_filepath, image_filepath, patient_text=None, audio_choice="Both"):
    ...
    download_file_urdu = urdu_audio_path
    download_file_english = english_audio_path

    return speech_text, doctor_response, urdu_response, english_audio_path, urdu_audio_path, download_file_english,download_file_urdu

def replay_audio(audio):
    return audio
...
def launch_gradio():
    with gr.Blocks(title="AI Doctor with Vision and Voice + Urdu") as demo:
        ...
        gr.Markdown("👋 **Click the button below to begin. The AI doctor will greet you.**")

        with gr.Row():
            start_btn = gr.Button("🚀 Start AI Doctor")

        welcome_audio = gr.Audio(value=None, label="🔊 Welcome Voice", autoplay=True, interactive=False, visible=True)

        # Container for the main UI – hidden initially
        with gr.Column(visible=False) as main_ui:
            ...
            audio_output_english = gr.Audio(label="🔊 Doctor's Voice in English", interactive=False, autoplay=True)
            audio_output_urdu = gr.Audio(label="🔊 Doctor's Voice in Urdu", interactive=False)
            
            with gr.Row(variant="compact"):
                download_btn_english = gr.File(label="📁 Download Doctor Audio in English")
                download_btn_urdu = gr.File(label="📁 Download Doctor Audio in Urdu")
            
            with gr.Row():
                replay_btn_english = gr.Button("🔁 Replay English Voice")
                replay_btn_urdu = gr.Button("🔁 Replay Urdu Voice")

            replay_btn_english.click(
                fn=lambda audio: (None, audio),
                inputs=[audio_output_english],
                outputs=[audio_output_english, audio_output_english]
            )

            replay_btn_urdu.click(
                fn=lambda audio: (None, audio),
                inputs=[audio_output_urdu],
                outputs=[audio_output_urdu, audio_output_urdu]
            )
            ...
            submit_btn.click(
                ...
                outputs=[
                    ...
                    audio_output_english, audio_output_urdu, download_btn_english, download_btn_urdu
                ]
            )

        # Start button logic: play welcome and show full UI
        start_btn.click(
            fn=lambda: (WELCOME_AUDIO_PATH, gr.update(visible=True)),
            inputs=[],
            outputs=[welcome_audio, main_ui]
        )
...
# store all response in one session
# after user response and doctor response complete A.I doctor ask again say "Thank You to end or is there anything else to ask?" until then wait for 1 minute and end the session
# use an AI face when audio play and return to interface when audio ends
# 